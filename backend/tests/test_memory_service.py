...
class TestMemoryQueries:
    ...
    async def test_store_and_retrieve_organizational_memory(self, setup_database):
        ...
        org_memories = await get_organizational_memories(limit=10)
    ...
    async def test_search_memories(self, setup_database):
        ...
        results = await search_memories("discount", limit=10)
        assert len(results) >= 1
...
class TestMemoryService:
    ...
    async def test_evaluate_and_store_promotable_event(self, setup_database):
        ...
        result = await service.evaluate_and_store(event)
        assert result is not None
        assert result["importance"] > 0.5

    async def test_evaluate_and_store_non_promotable_event(self, setup_database):
        service = MemoryService()
        event = {
            "case_id": str(uuid.uuid4()),
